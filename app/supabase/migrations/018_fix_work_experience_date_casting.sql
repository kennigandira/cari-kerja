-- Migration 018: Fix Work Experience Date Casting in RPC
-- Date: 2025-10-06
-- Description: Add NULLIF casting for work_experiences dates (remote DB uses DATE type)
-- Issue: work_experiences.start_date is DATE type but RPC passes TEXT

BEGIN;

-- Update create_master_profile RPC to cast work experience dates to DATE
...
BEGIN
  ...
  INSERT INTO master_profiles (
    user_id, session_id, profile_name, is_default, full_name, email,
    phone_primary, phone_secondary, linkedin_url, github_url, portfolio_url,
    location, professional_summary, years_of_experience, current_position
  )
  VALUES (
    v_user_id, v_session_id,
    COALESCE(p_profile->>'profile_name', 'Main Profile'),
    COALESCE((p_profile->>'is_default')::BOOLEAN, true),
    p_profile->>'full_name', p_profile->>'email',
    p_profile->>'phone_primary', p_profile->>'phone_secondary',
    p_profile->>'linkedin_url', p_profile->>'github_url', p_profile->>'portfolio_url',
    p_profile->>'location', p_profile->>'professional_summary',
    (p_profile->>'years_of_experience')::INTEGER,
    p_profile->>'current_position'
  )
  RETURNING id INTO v_profile_id;

  -- Insert work experiences (with DATE casting)
  IF jsonb_array_length(p_experiences) > 0 THEN
    FOR v_exp IN SELECT * FROM jsonb_array_elements(p_experiences)
    LOOP
      INSERT INTO work_experiences (
        profile_id, company_name, position_title, location,
  ...
  IF jsonb_array_length(p_education) > 0 THEN
    ...
    LOOP
      INSERT INTO education (
        profile_id, institution_name, degree_or_field, location, description,
        start_date, end_date, date_precision, is_current, display_order
      )
      VALUES (
        v_profile_id,
  ...
  IF jsonb_array_length(p_certifications) > 0 THEN
    ...
    LOOP
      INSERT INTO certifications (
        profile_id, certification_name, issuing_organization,
        credential_id, credential_url, description,
        issue_date, expiry_date, date_precision, display_order
      )
      VALUES (
        v_profile_id,
...
COMMENT ON FUNCTION create_master_profile IS 'Creates master profile with all related data atomically. Handles DATE casting for both VARCHAR and DATE column types.';
