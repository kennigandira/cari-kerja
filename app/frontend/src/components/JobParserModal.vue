...
// CSP-compliant progress class (rounds to nearest integer for discrete CSS classes)
const progressClass = computed(() => `progress-${Math.round(progress.value.progress)}`)
...
<template>
  <Teleport to="body">
    ...
    >
      <div class="bg-white rounded-lg shadow-xl w-full max-w-[42rem] max-h-[90vh] overflow-y-auto">
        <!-- Header -->
...
/* CSP-compliant progress bar width classes (0-100%) */
.progress-0 { width: 0%; }
.progress-1 { width: 1%; }
.progress-2 { width: 2%; }
.progress-3 { width: 3%; }
